/**
 * Envío de correos. Dos caminos, según lo que esté configurado:
 *
 *  1. GMAIL_USER + GMAIL_APP_PASSWORD → se envía por SMTP de Gmail. Escribe a cualquiera y no
 *     hace falta dominio propio. Es el modo en uso.
 *  2. RESEND_API_KEY → alternativa por API. OJO: sin un dominio verificado, Resend SOLO deja
 *     escribir al titular de la cuenta, así que no sirve para avisar a las clientas.
 *
...
async function enviarPorGmail({ para, asunto, html }) {
  try {
    ...
    console.error('[email] Gmail no pudo enviar:', err?.message || err);
    return false;
  }
}
